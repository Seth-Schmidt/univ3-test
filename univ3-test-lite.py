...
async def get_powerloom_data(pool_address: str):
    ...
    start_epoch = (last_finalized_epoch - epochs_in_range) + 1

    current_block = epoch_end

    print(f"Getting powerloom data for pool: {address}")
    print(f"Last finalized epoch: {last_finalized_epoch}")
    ...
    for i in reversed(range(start_epoch, last_finalized_epoch + 1)):
